...
const DashboardRenderer = ({ config, tickets = [], users = [] }) => {
  const computeMetric = (metric, sourceData) => {
    switch (metric) {
      ...
      case "reverify":
        return sourceData.filter((t) => t.status === TICKET_STATUS.REVERIFY)
          .length;

      case "submitted":
        return sourceData.filter((t) => t.status === TICKET_STATUS.SUBMITTED)
          .length;
      ...
      case "actionTaken":
        return sourceData.filter((t) => t.status === TICKET_STATUS.ACTION_TAKEN)
          .length;
